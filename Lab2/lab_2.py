...
class ForwardKinematics(Node):
    ...
    def forward_kinematics_f(self, theta1, theta2, theta3):
        # T_0_1 (base_link to leg_front_l_1)
        T_0_1 = self.translation(0.06, 0.053, 0) @ self.rotation_y(theta1)
        # T_1_2 (leg_front_l_1 to leg_front_l_2)
        T_1_2 =  self.translation(0, 0.045, 0) @ self.rotation_x(-math.pi / 2) @ self.rotation_y(theta2)
        # T_2_3 (leg_front_l_2 to leg_front_l_3)
        ## TODO: Implement the transformation matrix from leg_front_l_2 to leg_front_l_3
        T_2_3 =  self.translation(0.06, 0.06, 0) @ self.rotation_x(math.pi / 2) @ self.rotation_y(theta3)
        # T_3_ee (leg_front_l_3 to end-effector)
        ## TODO: Implement the transformation matrix from leg_front_l_3 to end effector
        T_3_ee = self.translation(0.067, 0, -0.067)
        # TODO: Compute the final transformation. T_0_ee is the multiplication of the previous transformation matrices
        ...
        print(f"End-Effector Position Test: x={end_effector_position[0]:.2f}, y={end_effector_position[1]:.2f}, z={end_effector_position[2]:.2f}")
        return end_effector_position
    ...
    def forward_kinematics_b(self, theta1, theta2, theta3):
        ...
        T_0_1 = self.translation(-0.075, 0.053, 0) @ self.rotation_y(theta1)
        ...
        # print(f"Back End-Effector Position Test: x={end_effector_position[0]:.2f}, y={end_effector_position[1]:.2f}, z={end_effector_position[2]:.2f}")
    ...
    def timer_callback(self):
        ...
        if self.joint_positions is not None:
            # Joint angles
            theta1_f = self.joint_positions[0] + 1.6605217742919922
            print(f"theta1_f: {theta1_f}")
            theta2_f = self.joint_positions[1] - 0.05569068908691405
            print(f"theta2_f: {theta2_f}")
            theta3_f = self.joint_positions[2] - 1.7608175659179688
            print(f"theta3_f: {theta3_f}")
            theta1_b = self.joint_positions[3] + 1.518994598388672
            print(f"theta1_b: {theta1_b}")
            theta2_b = self.joint_positions[4] + 0.02441890716552736
            print(f"theta2_b: {theta2_b}")
            theta3_b = self.joint_positions[5] - 1.7096996688842774
            print(f"theta3_b: {theta3_b}")
            ...
            if np.linalg.norm(end_effector_position_f - end_effector_position_b) < 0.02:  # If the distance is less than 2cm
                playing = sound.play()
            else:
                playing = None
            ...
            self.get_logger().info(
                ...
                f" | Back Leg End-Effector Position: x={end_effector_position_b[0]:.2f}, y={end_effector_position_b[1]:.2f}, z={end_effector_position_b[2]:.2f}"
            )
