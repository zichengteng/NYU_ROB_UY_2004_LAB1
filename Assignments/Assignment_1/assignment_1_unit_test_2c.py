...
def unit_test(test_number, rotation_list, p_point, q_point_des):
    q_point = my_assignment_1.rotate3D_many_times(rotation_list, p_point)
...
[ [[0, 'z'], [math.pi/2, 'z']], np.array([1,0,0]), np.array([0,1,0])  ],
[ [[math.pi/2, 'z'], [math.pi/2, 'x']], np.array([1,0,0]), np.array([0,0,1])  ],
]


######################### MAIN ##########################
print("Running unit tests for assignment 1.2c:")
...
for row in test_input_output_list:
    ...
    p_point = row[1]
    q_point_des = row[2]
    if unit_test(test_number, rotation_list, p_point, q_point_des):
