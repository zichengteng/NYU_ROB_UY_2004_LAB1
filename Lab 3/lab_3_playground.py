...
class InverseKinematics():
    ...
    def __init__(self):
        ...
        ################################################################################################
        # Triangle in the x-z plane (all local y = 0):
        # base width = 0.10 m, base center = [0, 0, -0.14], height = 0.09 m
        touch_down_position = np.array([0.07, 0.0, -0.14])
        stand_position_1 = np.array([0.035, 0.0, -0.14])
        stand_position_2 = np.array([0.0, 0.0, -0.14])
        stand_position_3 = np.array([-0.035, 0.0, -0.14])
...
def main():
    ...
    # Plot the cached trot gait path for the front-left foot.
    if len(inverse_kinematics.target_ee_cache):
        x_list = []
        z_list = []
        for position in inverse_kinematics.target_ee_cache:
            # target_ee_cache row layout: [RF xyz, LF xyz, RB xyz, LB xyz]
            x_list.append(position[3])
            z_list.append(position[5])
        plt.xlabel('X(m)')
        plt.ylabel('Z(m)')
